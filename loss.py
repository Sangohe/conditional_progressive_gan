...
def G_wgan_acgan(G, D, opt, training_set, minibatch_size, real_masks,
    ...
    # Repeat inputs for loss computation.
    repeated_latents = tf_repeat(latents, [repeats, 1])
    repeated_labels = tf.tile(tf_repeat(labels, [2, 1]), [repeats//2, 1])
    repeated_masks = tf.tile(real_masks[:minibatch_size], [repeats, 1, 1, 1])
    scaled_masks = repeated_masks * 0.5 + 0.5

    print(repeated_latents, repeated_labels, repeated_masks)

    # Generate images.
    fake_images_out = G.get_output_for(repeated_latents, repeated_labels, repeated_masks, is_training=True)

    # Pass the generated images to the discriminator and compute the WGAN loss.
    ...
    if D.output_shapes[1][1] > 0:
        with tf.name_scope('LabelPenalty'):
            label_penalty_fakes = tf.nn.softmax_cross_entropy_with_logits_v2(labels=repeated_labels, logits=fake_labels_out)
        loss += label_penalty_fakes * cond_weight

    if WT["texture_consistency"] > 0:
        with tf.name_scope('TextureConsistency'):
            lista = []
            print(latents.shape.as_list())
            print(type(lista))
            print(type(repeats))
            print(type([minibatch_size]))
            partitions = [0, 0, 1, 1, 0, 0, 1, 1] * ((num * repeats) // 8)
            print("==============znpartitions\n=====================",partitions)
            fake_images_1, fake_images_2 = tf.dynamic_partition(fake_images_out, partitions, 2)
...
def D_wgangp_acgan(G, D, opt, training_set, minibatch_size, reals, labels, masks,
    ...
    fake_images_out = G.get_output_for(latents, labels, masks, is_training=True)
