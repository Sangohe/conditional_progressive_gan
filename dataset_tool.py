...
class TFRecordExporter:
    ...
    def add_image_and_mask(self, img, mask):
        if self.print_progress and self.cur_images % self.progress_interval == 0:
            print('%d / %d\r' % (self.cur_images, self.expected_images), end='', flush=True)
        if self.shape is None:
            self.shape = img.shape
        ...
        for lod, tfr_writer in enumerate(self.tfr_writers):
            if lod:
                ...
                mask = (mask[:, 0::2, 0::2] + mask[:, 0::2, 1::2] + mask[:, 1::2, 0::2] + mask[:, 1::2, 1::2]) * 0.25
            ...
            ex = tf.train.Example(features=tf.train.Features(feature={
                'shape': tf.train.Feature(int64_list=tf.train.Int64List(value=quant.shape)),
                'data': tf.train.Feature(bytes_list=tf.train.BytesList(value=[quant.tostring()])),
                'mask_shape': tf.train.Feature(int64_list=tf.train.Int64List(value=quant_mask.shape)),
...
def create_acdc_with_masks(tfrecord_dir, hdf5_filename, shuffle):
    print('Loading HDF5 archive from "%s"' % hdf5_filename)
    import h5py # conda install h5py
    ...
    labels = np.random.randint(0, 5, size=images.shape[0])
    labels = tf.keras.utils.to_categorical(labels)

    # Transpose both the masks and images.
    ...
    with TFRecordExporter(tfrecord_dir, images.shape[0]) as tfr:
        ...
        for idx in range(order.size):
            ...
            mask = masks[order[idx]]
            tfr.add_image_and_mask(image, mask)
...
def execute_cmdline(argv):
    ...
    p = add_command(    'create_acdc_with_masks', 'Create dataset from legacy HDF5 archive.',
                                            'create_acdc_with_masks datasets/celebahq ~/downloads/celeba-hq-1024x1024.h5')
    p.add_argument(     'tfrecord_dir',     help='New dataset directory to be created')
    p.add_argument(     'hdf5_filename',    help='HDF5 archive containing the images')
    p.add_argument(     '--shuffle',        help='Randomize image order (default: 1)', type=int, default=1)
